...
@section('content')
  ...
  <h2 class="text-xl my-2"><svg class="w-6 h-6 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    </svg> Vos propositions ({{ auth()->user()->proposal->count() }})</h2>
    {{ auth()->user()->name }}
     @foreach(auth()->user()->proposal as $proposals)
    <div class="mb-3 {{ $proposals->validated ? 'text-green-400' : '' }}">
    <span class="block font-semibold items-center">
       Pour la mission "{{ $proposals->job->title }}"
    </span>
     {{ $proposals->coverletter ? $proposals->coverletter->content : 'No cover letter available' }}
    </div>
    @endforeach
  </section>
  ...
  <section class="text-sm text-gray-700 w-full">
    ...
    <div class="mb-3">
      <span class="block font-semibold">
        {{ $job->title }} ({{ $job->proposal->count() }} @choice('proposition|propositions', $job->proposal)) :</span>
      </span>
      ...
      <span class="bg-white border border-green-500 text-xs p-1 my-2 inline-block text-green-500 rounded">Déjà validé</span>
       @else
      <a href="/acceptProposal/{{ $proposal->id }}" class="bg-green-500 text-xs py-2 px-2 mt-2 mb-3 inline-block text-white hover:bg-green-200 hover:text-green-500 duration-200 transition rounded">Valider la proposition</a>
      @endif
       @endforeach
      </ul>
    </div>
